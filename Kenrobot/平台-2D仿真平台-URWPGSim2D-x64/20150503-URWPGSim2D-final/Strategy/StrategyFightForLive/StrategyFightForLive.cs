    ...
    public class Strategy : MarshalByRefObject, IStrategy
    {
        ...
        public Decision[] GetDecision(Mission mission, int teamId)
        {
           ...
           //fishPth = mission.TeamsRef[teamId].Fishes[0].PositionMm;
            #endregion
            xna.Vector3 fishPt = new xna.Vector3();
            fishPt = mission.TeamsRef[teamId].Fishes[0].PositionMm;//定义零号鱼的坐标
            xna.Vector3 HC = new xna.Vector3(0f, 0f, -360f);//定义转折点
            xna.Vector3 fishPth = new xna.Vector3();
            fishPth = mission.TeamsRef[(1+teamId)%2].Fishes[1].PositionMm;//定义对方三号鱼的坐标、
            if (fishPt.X < 0)
            {
                PointToPoint(0, mission.TeamsRef[teamId].Fishes[0], HC);
            }
            else
            {
                PointToPoint(0, mission.TeamsRef[teamId].Fishes[0], fishPth);
            }
          
            return decisions;
        }
        //点对点函数
        public void PointToPoint(int i,RoboFish fish ,xna.Vector3 goal)
        {

            float dirgoal = DirfishToG(fish.PositionMm, goal);
            float diryu = fish.BodyDirectionRad;
            float disgoal = DisFishToBall(fish.PositionMm, goal);
            float cha=dirgoal -diryu ;
            if (disgoal > 300)
            {
                if((Math .Abs (cha ))<=Math .PI /30)
                {
                    decisions[i ].VCode = 14; 
                    decisions[i ].TCode = 7;
                }
                else if (cha < -Math.PI / 30 && cha > -Math.PI / 15)
                {
                    decisions[i ].VCode = 9;
                    decisions[i ].TCode = 6;
 
                }
                else if (cha < Math.PI / 15 && cha > Math.PI / 30)
                {
                    decisions[i ].VCode = 9;
                    decisions[i ].TCode = 8;
                }
                else if (cha < -Math.PI / 15 && cha > -Math.PI / 6)
                {
                    decisions[i ].VCode = 7;
                    decisions[i ].TCode = 4;
                }
                else if (cha < Math.PI / 6 && cha > Math.PI / 15)
                {
                    decisions[i ].VCode = 7;
                    decisions[i ].TCode = 9;
                }
                else if (cha < -Math.PI / 6 && cha > -Math.PI / 3)
                {
                    decisions[i ].VCode = 5;
                    decisions[i ].TCode = 3;
                }
                else if (cha < Math.PI / 3 && cha > Math.PI / 6)
                {
                    decisions[i ].VCode = 5;
                    decisions[i ].TCode = 10;
 
                }
                else if (cha < -Math.PI / 3 && cha > -Math.PI / 2)
                {
                    decisions[i ].VCode = 4;
                    decisions[i ].TCode = 2;
                }
                else if (cha < Math.PI / 2 && cha > Math.PI / 3)
                {
                    decisions[i ].VCode = 4;
            ...
            else if (disgoal < 300 && disgoal > 40)
            {
                decisions[i].VCode = 7;
                decisions[i].TCode = 7;
       ...
       public float DirfishToG(xna.Vector3 fishPt, xna.Vector3 goalPt)
        {
            ...
            {
                float dirFishToG = (float)(Math.Atan((fishPt.Z - goalPt.Z) / (fishPt.X - goalPt.X)) + Math.PI);
                return dirFishToG;
            }
